...
if(isset($_SESSION["r_id"]) && $_SESSION["type"] == "researcher" && isset($_REQUEST["btnSearch"])){
    ...
    $query= "SELECT qs.question_set_id AS SET_NO , qsn.name AS question FROM question_set AS qs JOIN question_set_name AS qsn ON qs.question_set_id= qsn.question_set_id WHERE qs.researcher_id =$r_id AND qsn.name like '$match'";
