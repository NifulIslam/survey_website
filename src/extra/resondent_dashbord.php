...
$query ="SELECT * FROM questions WHERE (resp_age=0 OR resp_age = '$r_age') AND (resp_gender= 'NULL' OR resp_gender= '$r_gend') AND (resp_social_statuc= 'NULL' OR resp_social_statuc = '$r_sc') AND  (resp_nationality ='' OR resp_nationality= '$r_nat') AND to_reach !=0 AND question_id  NOT IN (SELECT question_id FROM responce WHERE respondent_id ='$id')";
$result = mysqli_query($connection, $query);
